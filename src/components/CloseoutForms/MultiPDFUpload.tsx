...
import { Button } from '@/components/ui/button';
import { Label } from '@/components/ui/label';
import { Upload, File, CheckCircle, RefreshCw, Database, AlertCircle } from 'lucide-react';
import { Client } from './ClientSearch';
...
const MultiPDFUpload = ({ client, onDataExtracted }: MultiPDFUploadProps) => {
  ...
  const handleDataExtraction = async () => {
    ...
    try {
      let finalExtractedData: any = null;
      let successCount = 0;
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          console.log(`Extracting data from PDF ${i + 1}:`, uploadedFile.name);
          const extractedData = await extractPDFData(uploadedFile.file);
          console.log(`Extracted data from ${uploadedFile.name}:`, extractedData);
          
          // Update file status to processed
          setUploadedFiles(prev => 
            prev.map((f, index) => 
              index === i ? { ...f, status: 'processed' } : f
            )
          );
          
          // Use first successful extraction as base, merge others if needed
          if (!finalExtractedData) {
            finalExtractedData = extractedData;
          } else {
            // Merge family members from multiple PDFs if needed
            if (extractedData.familyMembers && extractedData.familyMembers.length > 0) {
              finalExtractedData.familyMembers = [
                ...(finalExtractedData.familyMembers || []),
                ...extractedData.familyMembers.map((member: any, idx: number) => ({
                  ...member,
                  id: String(finalExtractedData.familyMembers.length + idx + 1)
                }))
              ];
            }
          }
          ...
          console.error(`Failed to extract from ${uploadedFile.name}:`, error);
      ...
      if (successCount > 0 && finalExtractedData) {
        setExtractionComplete(true);
        onDataExtracted(finalExtractedData);
      } else {
        // All extractions failed - offer demo data
        if (window.confirm('PDF extraction failed for all files. Would you like to proceed with demo data instead?')) {
          const demoData = generateDemoData(client);
          setExtractionComplete(true);
      ...
      if (window.confirm('PDF extraction failed. Would you like to proceed with demo data instead?')) {
        const demoData = generateDemoData(client);
        setExtractionComplete(true);
        onDataExtracted(demoData);
      }
  ...
  const generateDemoData = (client: any) => {
    return {
      filePath: `\\\\Clearhouse\\Clients\\${client.name?.replace(' ', '_') || 'NewClient'}_2024\\T1`,
      partner: 'Priya S.',
      manager: 'Deepak Jain',
      ...
      t1032PensionSplit: Math.random() > 0.8,
      hstDraftOrFinal: Math.random() > 0.5 ? 'Final' : 'Draft',
      otherNotes: 'Demo data - please update with actual information',
      priorPeriodsBalance: Math.random() > 0.5 ? `${Math.floor(Math.random() * 1000)}` : '0',
      taxesPayable: `${Math.floor(Math.random() * 5000 - 2000)}`,
      installmentsDuringYear: `${Math.floor(Math.random() * 3000)}`,
      installmentsAfterYear: `${Math.floor(Math.random() * 1000)}`,
